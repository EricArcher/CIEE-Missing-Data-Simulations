landscape.new.ind.genos <- function(rland, PopulationSizes, AlleleFreqs = NULL) {
  ...
  loccols <- length(landscape.locusvec(rland))
  rland$individuals <- matrix(
    nrow = sum(PopulationSizes),
    ncol = landscape.democol() + length(landscape.locusvec(rland))
  )
  ...
  for (i in 1:length(AlleleFreqs)) {
    ...
    cols <- landscape.democol() + which(landscape.locusvec(rland) == i)
...
loadLandscape <- function(sc, AlleleFreqs, num.gens) {
  rl <- landscape.new.empty()

  rl <- landscape.new.intparam(
    rl, h = sc$num.pops, s = 2, cg = 0, ce = 0, totgen = num.gens + 1
  )

  rl <- landscape.new.switchparam(rl)
  rl <- landscape.new.floatparam(rl)

  localS <- matrix(c(0, 1, 0, 0), nrow = 2, ncol = 2)
  localR <- matrix(c(0, 0, 1.2, 0), nrow = 2, ncol = 2)
  localM <- matrix(c(0, 0, 0, 1.2), nrow = 2, ncol = 2)
  rl <- landscape.new.local.demo(rl, localS, localR, localM)

  S <- M <- matrix(0, nrow = sc$num.pops * 2, ncol = sc$num.pops * 2)
  diag(S) <- diag(M) <- 1
  R <- landscape.mig.matrix(
    h = nrow(sc$mig.mat), s = 2, mig.model = "custom", R.custom = sc$mig.mat
  )$R
  rl <- landscape.new.epoch(rl, R = R, carry = rep(sc$Ne, sc$num.pops))

  #just make loci that have the correct type and ploidy
  for (i in 1:length(AlleleFreqs)) {
    rl <- landscape.new.locus(
...
landscape2gtypes <- function(Rland) {
  ...
  gen.data <- Rland$individuals[, -(1:landscape.democol())]
  ...
  df2gtypes(gen.data, ploidy = pl[1], id.col = NULL, strata.col = 1, loc.col = 2)
}
